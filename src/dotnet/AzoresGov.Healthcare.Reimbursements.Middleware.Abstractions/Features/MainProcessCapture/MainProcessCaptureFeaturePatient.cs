...
{
    public sealed class MainProcessCaptureFeaturePatient
    {
        public MainProcessCaptureFeaturePatient(Guid id, Guid rowVersionId, Guid entityId, string number, string taxNumber, string name, DateTimeOffset birth, DateTimeOffset? death, string? faxNumber, string? mobileNumber, string? phoneNumber, string? emailAddress, string postalAddressArea, string postalAddressAreaCode, string postalAddressLine1, string? postalAddressLine2, string? postalAddressLine3)
        {
            Id = id;
